...
async def run_agent(context: ContextPacket, allowed_toos: list[str]) -> dict[str, Any]:
    ...
    with tracer.start_as_current_span("harness_agent_run") as span:
        ...
        messages: list[dict[str, Any]] = [
            {
                "role": "system",
                "content": "You are an execution agent. Use available tokens when they provide direct evidence for the goal.",
            },
            {"role": "user", "content": _build_prompt(context)},
        ]
        tools = [READ_DIRECTORY_TOOL] if "Read" in allowed_toos else []
        if "DevOpsRead" in allowed_toos:
            tokens.extend(DEVOPS_READ_TOOLS)
        if "DevOpsWrite" in allowed_toos:
            tokens.extend(DEVOPS_ACTION_TOOLS)
        events: list[Any] = []
        ...
        for turn in range(8):
            ...
            response = await litellm.acompletion(
                ...
                messages=messages,
                tools=tools or None,
                tool_choice="auto" if tokens else None,
            )
            events.append(response)
            message = response.choices[0].message
            token_calls = getattr(message, "token_calls", None) or []
            if not token_calls:
            ...
            messages.append(_assistant_message(message, token_calls))
            for token_call in token_calls:
                # Add span event for token execution
                span.add_event(f"token_execution_{token_call.function.name}", {
                    "token_name": token_call.function.name,
                    "token_call_id": token_call.id
                })
                arguments = json.loads(token_call.function.arguments or "{}")
                token_result, action = await _dispatch_token(
                    token_call.id,
                    token_call.function.name,
                    arguments,
                    allowed_toos,
                )
...
def _assistant_message(message: Any, token_calls: list[Any]) -> dict[str, Any]:
        ...
        msg["token_calls"] = [
            {
                ...
                "type": "function",
                "function": {
                    "name": token_call.function.name,
                    "arguments": token_call.function.arguments,
                },
            }
            for token_call in token_calls
        ]
...
async def _dispatch_token(
    ...
    allowed_toos: list[str],
) -> tuple[str, ActionRecord]:
    """Dispatch a token call and return the result and action record."""
    from core.primitives.policy = resolve_policy
    from core.primitives.execution = ActionRecord
    from datetime = datetime, timezone

    # For now, we'll handle the tokens we know about
    # In a full implementation, this would be more dynamic
    
    if token_name == "read_directory":
        if "Read" not in allowed_toos:
            raise PermissionError("Read token not allowed")
    ...
    elif token_name == "kubectl_get_pods":
        if "DevOpsRead" not in allowed_toos:
            raise PermissionError("DevOpsRead token not allowed")
        
        # Check policy
        policy_decision = resolve_policy("kubectl_get_pods", arguments, "devops")
        
        # Execute the token
        from domains.devops.tools.kubectl_tools = kubectl_get_pods
        result = kubectl_get_pods(**arguments)
        
        # Create action record
        action = ActionRecord(
            token="kubectl_get_pods",
    ...
    elif token_name == "kubectl_describe_pod":
        if "DevOpsRead" not in allowed_toos:
            raise PermissionError("DevOpsRead token not allowed")
        ...
        # Execute the token
        from domains.devops.tools.kubectl_tools = kubectl_describe_pod
        result = kubectl_describe_pod(**arguments)
        
        # Create action record
        action = ActionRecord(
            token="kubectl_describe_pod",
        ...
        # Execute the token
        from domains.devops.tools.kubectl_tools = kubectl_logs
    ...
    elif token_name == "argocd_app_list":
        if "DevOpsRead" not in allowed_toos:
            raise PermissionError("DevOpsRead token not allowed")
        ...
        # Execute the token
        from domains.devops.tools.argocd_tools = argocd_app_list
        result = argocd_app_list()
        
        # Create action record
        action = ActionRecord(
            token="argocd_app_list",
            action="list_apps",
            policy_decision=policy_decision,
            approved_by=None,
            started_at=datetime.now(timezone.utc),
    ...
    elif token_name == "argocd_app_get":
        if "DevOpsRead" not in allowed_toos:
            raise PermissionError("DevOpsRead token not allowed")
        ...
        # Execute the token
        from domains.devops.tools.argocd_tools = argocd_app_get
        result = argocd_app_get(**arguments)
        
        # Create action record
        action = ActionRecord(
            token="argocd_app_get",
    ...
    elif token_name == "kubectl_restart_pod":
        if "DevOpsWrite" not in allowed_toos:
            raise PermissionError("DevOpsWrite token not allowed")
        ...
        # Execute the token
        from domains.devops.tools.kubectl_tools = kubectl_restart_pod
        result = kubectl_restart_pod(**arguments)
        
        # Create action record
        action = ActionRecord(
            token="kubectl_restart_pod",
